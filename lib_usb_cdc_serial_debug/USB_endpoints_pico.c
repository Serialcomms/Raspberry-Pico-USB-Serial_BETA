...
static inline void build_pico_function_endpoint(uint8_t EP_NUMBER, uint16_t TRANSFER_TYPE, uint16_t QUEUE_SIZE, void *endpoint_handler) {
  ...
  pico_endpoint[EP_NUMBER].ep_busy = false;
  pico_endpoint[EP_NUMBER].queue_busy = false;
  pico_endpoint[EP_NUMBER].queue_long = false;
  ...
  uart_printf("Build Pico Function Endpoint %d, Queue Size=%d, Endpoint Control=%08X\n\r", EP_NUMBER, QUEUE_SIZE, ep_control_register);
...
static inline void build_pico_default_endpoint(uint16_t QUEUE_SIZE, void *endpoint_handler) {
  ...
  pico_endpoint[0].ep_busy = false;
  pico_endpoint[0].queue_busy = false;
  pico_endpoint[0].queue_long = false;
  ...
  uart_printf("Build Pico Default Endpoint %d, DPRAM Address=%08X\n\r", 0, usb_dpram_address);
