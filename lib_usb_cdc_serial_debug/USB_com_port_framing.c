...
uint8_t parity_text(uint8_t cdc_parity_value) {

  const uint8_t valid_parity_text[] = {'N', 'O', 'E', 'M', 'S'};

  const uint8_t *parity_text = valid_parity_text;

  if (cdc_parity_value < count_of(valid_parity_text)) {

    return parity_text[cdc_parity_value];
...
const uint8_t *stop_text(uint8_t cdc_stop) {
  ...
  const uint8_t *unknown_text[] = {"?"};

  const uint8_t *valid_stop_text[] = {"1" , "1.5" , "2"};

  if (cdc_stop < count_of(valid_stop_text)) {

    return valid_stop_text[cdc_stop];

  } else {
    
    return unknown_text[0];
