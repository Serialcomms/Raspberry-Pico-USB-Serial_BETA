...
bool queue_is_full_to_host(uint8_t EP_NUMBER) {

  return queue_is_full(&host_endpoint[EP_NUMBER].data_queue);

}

bool queue_is_full_to_pico(uint8_t EP_NUMBER) {

  return queue_is_full(&pico_endpoint[EP_NUMBER].data_queue);
...
bool queue_wait_for_capacity_to_host(uint8_t EP_NUMBER, uint16_t required_capacity, uint16_t wait_milliseconds) {
