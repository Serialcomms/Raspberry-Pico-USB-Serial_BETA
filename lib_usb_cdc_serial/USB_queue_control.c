...
void purge_endpoint_queue_to_host(uint8_t EP_NUMBER){

  bool removed;
  
  do {

    removed = queue_try_remove(&host_endpoint[EP_NUMBER].data_queue, NULL);

  } while (removed);

}

void purge_endpoint_queue_to_pico(uint8_t EP_NUMBER){

  bool removed;
  
  do {

    removed = queue_try_remove(&pico_endpoint[EP_NUMBER].data_queue, NULL);
