...
static inline void send_text_string(uint8_t string_index) {
    ...
    if (string_index < count_of(pico_string_descriptors)) {
        ...
        uart_printf("Sending Text String, Index=%d, Length=%d, String=%s\n\r", 
        ...
        uart_printf("String index %d out of bounds\n\r", string_index);
...
static inline void send_language_string() {
    ...
    uart_printf("Sending Language String, Index=%d, bytes=%02x, %02x\n\r", 
...
static inline uint8_t usb_convert_string_descriptor(uint8_t string_index) {
    ...
    uart_printf("String index=%d, string length=%d, buffer length=%d\n\r", string_index, string_length, buffer_length) ;
